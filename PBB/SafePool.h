#ifndef spsSMPThreadPool_h
#define spsSMPThreadPool_h
 ...
 */
class PBB_EXPORT spsSMPThreadPool
{
  ...
  public:
     ...
     * the creating thread or in a thread that does not belong to the pool, otherwise it may create
     * a deadlock when joining.
     */
    class PBB_EXPORT Proxy final
    {
      public:
        /**
         * @brief Destructor
        ...
        /**
         * @brief Return true is this proxy is allocated from a thread that does not belong to the
         * pool
         */
        bool IsTopLevel() const noexcept;

      private:
        friend class spsSMPThreadPool; // Only the thread pool can construct this object
    ...
    spsSMPThreadPool();
    ~spsSMPThreadPool();
    spsSMPThreadPool(const spsSMPThreadPool&) = delete;
    spsSMPThreadPool& operator=(const spsSMPThreadPool&) = delete;
  ...
  public:
    static spsSMPThreadPool& GetInstance();
};

} // namespace detail
} // namespace PBB

/* VTK-HeaderTest-Exclude: spsSMPThreadPool.h */
